...
import game.model.assemblers.BattleAssembler;
...
public class BattleInvitationResponseCommand implements BaseCommand<BattleInvitationResponseDTO> {
    ...
    public void execute(BattleInvitationResponseDTO response, ObjectOutputStream outputStream, SessionObject sessionObject) {
        ...
        if (battle != null) {
            ...
            if (userMap.get(sessionObject.getUser()) != null) {
                sendResponseInformation(response, sessionObject, userMap);
                if(battle.allAccepted()){
                    ServerBroadcasting.broadcastBattleState(BattleAssembler.toBattleStateDTOs(battle));
                }
            } else {
    ...
    private void sendResponseInformation(BattleInvitationResponseDTO response, SessionObject sessionObject, Map<User, Battle.Status> userMap) {
